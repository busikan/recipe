...
async function sendToGPT() {
  ...
  try {
      ...
      // Check if the content is flagged as dangerous
      if (data.error && data.error === "The generated content contains inappropriate or dangerous elements") {
          alert("The generated content contains inappropriate or dangerous elements.");
          return; // Stop further execution
      }

      let recipes = data.recipes;

      // If the recipes are a string, split into an array
      if (typeof recipes === 'string') {
          recipes = recipes.split('\n\n');
      }

      displayRecipes(recipes);  // Display generated recipes
  } catch (error) {
